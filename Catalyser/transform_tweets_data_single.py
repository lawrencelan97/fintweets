...
with beam.Pipeline('DirectRunner', options=opts) as p:
    ...
    query_results = p | beam.io.Read(beam.io.BigQuerySource(query='select * from tweets.tweets_data LIMIT 100'))

    # write PCollection to a log file
    query_results | 'Write to File 1' >> WriteToText('input.txt')

    # apply a ParDo to the PCollection
    out_pcoll = query_results | 'Format DOB' >> beam.ParDo(FormatTimestampFn())

    # write PCollection to a log file
    out_pcoll | 'Write to File 2' >> WriteToText('output.txt')

    qualified_table_name = 'avian-force-216105:beam_dataset.Formatted_Timestamp_Milestone8_Single'
